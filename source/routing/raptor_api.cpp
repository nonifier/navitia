...
static std::vector<Path> call_raptor(navitia::PbCreator& pb_creator,
    ...
    for (const auto& datetime : datetimes) {
        ...
        if (datetimes.size() == 1) {
            pathes = tmp_pathes;
            LOG4CPLUS_DEBUG(logger, "** List Kraken journeys **");
            for (auto& path : pathes) {
                LOG4CPLUS_DEBUG(logger, path);
...
void make_pt_response(navitia::PbCreator& pb_creator,
    ...
    // Call Raptor loop
    const auto pathes =
        call_raptor(pb_creator, raptor, departures, arrivals, datetimes, rt_level, transfer_penalty,
